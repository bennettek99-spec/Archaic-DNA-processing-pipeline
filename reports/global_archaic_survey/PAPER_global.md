...
We extend a validated Neanderthal/Denisovan introgression pipeline from ancient Eurasians to the **entire Allen Ancient DNA Resource** — 21,109 quality-passing present-day and ancient genomes across Africa, the Americas, Eurasia, and Oceania — to ask which individuals, on any continent, carry Neanderthal ancestry above 5% and where relative Denisovan affinity is elevated. Three results follow. (1) **The out-of-Africa negative control holds:** African genomes read a mean Neanderthal proportion of 0.29% (median 0.09%) and the *most negative* Denisovan affinity of any continent, confirming the estimator's zero-point on data never used to calibrate it. (2) **No continent contains a single high-confidence genome exceeding 5% Neanderthal ancestry** — Americas, Eurasia, and Oceania all sit at a ~1.9–2.3% Neanderthal baseline, and the 120 raw >5% calls worldwide are, as in the Eurasia-only analysis, low-coverage artifacts (0 survive the high-confidence filter). (3) **Oceania has the strongest global archaic profile:** it carries the highest Neanderthal ancestry (2.3%) *and* a Denisovan affinity elevated far above every other region — **31.8% of Oceanian genomes (77 high-confidence) exceed the Denisovan flag**, led by Papuans (relative affinity Z up to 7.2), Bougainville, and Australians. Published studies estimate approximately 4–5% Denisovan ancestry in Papuans; combining that external literature value with the Neanderthal estimate would exceed 5%, but this pipeline does **not** calculate a Denisovan or combined archaic percentage. The genome-wide picture from this analysis is therefore: Neanderthal ancestry alone never exceeds ~3–4% outside of a recent-admixture individual, and high relative Denisovan affinity is concentrated in Oceania and Island Southeast Asia.
...
Surveyed across the whole AADR, archaic ancestry is globally coherent: Africans provide the near-zero control, the rest of Eurasia and the Americas sit at a ~2% Neanderthal baseline that no high-confidence genome exceeds by 5%, and **Oceania combines the highest Neanderthal estimate with the only strongly elevated Denisovan-affinity distribution** (Papuans, Bougainville, Australians). Published Denisovan percentages place combined ancestry above 5% in some living Oceanian populations, but that combined percentage is external context rather than an output of this pipeline. The tables (continental breakdown, worldwide >5% list, Oceanian Denisovan candidates) and figures are provided for follow-up.
