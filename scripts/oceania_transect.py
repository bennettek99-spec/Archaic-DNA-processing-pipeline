...
Run: PYTHONIOENCODING=utf-8 python scripts/oceania_transect.py --panel 1240k
"""
import argparse
import os
import sys
...
def _transversion_note(args, tv):
    ...
    if not (os.path.exists(path) and os.path.exists(coh)):
        return ("Not run. Execute `python scripts/oceania_transect.py --panel 1240k "
...
def write_report(df, mixdf, cmdf, indiv, trends, did, shared, args, tag):
    ...
    md = [
        ...
        "## Reproduce\n",
        "```bash\n"
        f"python scripts/oceania_transect.py --panel {args.panel}\n"
        f"python scripts/oceania_transect.py --panel {args.panel} --transversions\n"
        "```\n",
