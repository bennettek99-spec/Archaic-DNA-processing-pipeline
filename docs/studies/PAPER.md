...
As an independent positive control, the pipeline's Denisovan estimator D_Den = D(X, Mbuti; Denisova, Chimp) recovers the known Denisovan ancestry of present-day Oceanians (Papuan Z=+5.9, Nasioi Z=+5.1) and reads ~0 in East Asians, West Eurasians and Africans. Against that calibrated scale, ancient Eurasians carry no detectable Denisovan ancestry: every regional pool is within noise of zero (all |Z| < 1) and no individual exceeds the genome-wide Bonferroni outlier threshold. This controlled null — a real absence, not a lack of power — is the Denisovan counterpart to the genome-wide Neanderthal result and confirms the estimator flags archaic ancestry only where it truly occurs. The high-Denisovan Oceanian and Island-Southeast-Asian populations are absent from the ancient AADR Eurasian sampling, which is why the ancient signal is a null rather than a gradient.

| population | n | Denisovan D_Den (%) | Z |
|---|---|---|---|
| Papuan | 32 | +3.45 | +5.9 |
| Nasioi (Bougainville) | 13 | +2.46 | +5.1 |
| Han | 46 | -0.34 | -0.8 |
| French | 31 | -0.16 | -0.4 |
| Yoruba (African) | 24 | -0.50 | -2.0 |
| Ancient Eurasia — West | 10897 | -0.19 | -0.5 |
| Ancient Eurasia — Central/South | 2687 | -0.22 | -0.6 |
| Ancient Eurasia — East | 1808 | -0.22 | -0.6 |
