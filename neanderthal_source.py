...
def write_report(df, rdf, cdf, pwdf, cmdf, covdf, fit, cm_fit, lim, limR, gain,
    ...
    md = [
        ...
        f"disagree. Published estimates put East Asians somewhere around 8-20% "
        f"above Europeans in Neanderthal ancestry; this repository's f4-ratio "
        f"on 1240K puts the ratio at about 1.02, and D_NEA here gives "
        f"{get('PD_Han','D_NEA'):.4f} for Han against "
        f"{get('PD_French','D_NEA'):.4f} for French. The correction is "
        f"therefore small for the East Asian comparison specifically - but it "
        f"is large for the Palaeolithic cohorts and very large for Oase1, "
        f"which is where it changes the conclusion.\n",
