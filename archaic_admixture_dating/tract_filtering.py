...
def filter_tracts(
    ...
    *,
    ...
    confidence = work["posterior_denisovan"].where(
        work["posterior_denisovan"].notna(),
        work.get("posterior_archaic", pd.Series(np.nan, index=work.index)),
    )
    confidence_known = confidence.notna()
    flag(confidence_known & confidence.lt(minimum_confidence), "below_confidence")
