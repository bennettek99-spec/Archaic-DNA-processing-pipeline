...
document: docs/studies/PAPER.md + reports/etruscan_paper.html (figures fig_p1..fig_p4).
"""
...
def main():
    ...
    print("\nWrote docs/studies/PAPER.md + reports/etruscan_paper.html + results/etruscan/P_*.csv")
...
def write_paper(G, Cc, T, etr, o_absz, no_absz, Cl, n_tested, bonf, counts, figs):
    ...
    md = MD_TEMPLATE.format(
        ...
        locus_table="\n".join(
            ...
            for _, r in Cl.iterrows() if np.isfinite(r.get("age_coef_per_kyr", np.nan))))
    open(os.path.join(HERE, "docs", "studies", "PAPER.md"), "w", encoding="utf-8").write(md)
...
**Ancestry, relatedness and validation.** Ancestry is quantified by qpAdm (sources Anatolian Neolithic + Steppe/Yamnaya + WHG; distal outgroups Mbuti, Han, Papuan, Karitiana, Iran_N, Natufian, Ust'-Ishim, Mal'ta). Relatives and duplicate libraries are removed before group estimates with a READ-style pairwise-allele-mismatch scan (Monroy Kuhn et al. 2018). The estimator is additionally validated against msprime coalescent simulations with a *known* introgression fraction (calibration 0.97·true + 0.01 pp; the outlier detector's null false-positive rate matches the nominal level — docs/studies/SIMULATION_VALIDATION.md), and cross-checked against ADMIXTOOLS 2 (Maier et al. 2023) computed on a PLINK export: f-statistics correlate at r≥0.99 (absolute scales differ by a normalisation constant) and qpAdm ancestry proportions agree to within ~4 percentage points (Figure 6). This concordance check also caught and fixed an exporter encoding bug, illustrating its value.
...
<footer>Methods, validation and code: <code>README.md</code>, <code>docs/studies/VALIDATION.md</code>, <code>FINDINGS.md</code>, <code>docs/studies/PAPER.md</code>. Data: AADR (Mallick et al. 2024). Generated by <code>etruscan_paper.py</code>.</footer>
</div></body></html>"""
