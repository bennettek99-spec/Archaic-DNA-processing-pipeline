...
def test_bootstrap_max_tracts_is_reproducible_and_bounded():
    frame = pd.DataFrame(
        {
            "chromosome": ["1"] * 20 + ["2"] * 20,
            "length_cm": np.linspace(0.03, 1.2, 40),
        }
    )

    def count_tracts(lengths_cm):
        return {"n_tracts": len(lengths_cm)}
    ...
    pd.testing.assert_frame_equal(first, second)
    assert first["n_tracts"].eq(10).all()
