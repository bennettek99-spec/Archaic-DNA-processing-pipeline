...
def write_report(
    ...
    *,
    ...
    status = interpretation_status(warnings)
...
Under the tested tract-length models, **{best['model_name']}** has the lowest
...
recombination map, generation-time assumptions, and simulated alternatives. It
does **not** establish that Denisovans survived until the younger fitted date.
...
Under the tested models, the observed tract distribution is more compatible
...
plausible. Direct late Denisovan admixture is not demonstrated by a young
tract-length estimate alone.
