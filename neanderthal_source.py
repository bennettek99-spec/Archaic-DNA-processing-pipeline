...
Run: PYTHONIOENCODING=utf-8 python neanderthal_source.py --panel 1240k
"""
import argparse
import itertools
import os
...
def write_report(df, rdf, cdf, pwdf, cmdf, covdf, fit, cm_fit, lim, limR, gain,
    ...
    md = [
        ...
        f"The confound that does *not* cancel is how much Neanderthal ancestry "
        f"each cohort has, because D_VA scales with it. Each cohort is "
        f"therefore also measured with `D_NEA = D(X, Yoruba; NeaAvg, Chimp)`, "
        f"where `NeaAvg` is the mean of the two archaic frequencies. Swapping "
        f"Vindija for Altai leaves `NeaAvg` unchanged, so `D_NEA` tracks the "
        f"amount of Neanderthal ancestry without responding to its source. "
        f"Under a single shared source every cohort satisfies "
        f"`D_VA = k * D_NEA` with one k, and k estimates the Vindija preference "
        f"of the source population itself.\n",
        f"It is worth recording how large this confound actually is on this "
        f"panel, because the literature figure and this panel's own answer "
        f"disagree. All the quantities here are **relative**: the claim is that "
        f"East Asians carry proportionally more Neanderthal ancestry than "
        f"Europeans, not that they carry more in absolute percentage points of "
        f"the genome. In absolutes the whole argument lives between about 1.7% "
        f"and 2.6%, and the disputed gap is a few tenths of a percentage "
        f"point.\n",
        ...
        f"That this panel sits at the far end of a trend the field has itself "
        f"been moving along is worth noting rather than dismissing, and the "
        f"reason is directly relevant here: the estimate is measured against "
        f"Yoruba, and Yoruba is not archaic-free (see above) - the same effect "
        f"Chen et al. were correcting for. The practical consequence for this "
        f"study is that the quantity correction is **small for the East Asian "
        f"comparison specifically**, which is the one the literature argues "
        f"about, but large for the Palaeolithic cohorts and very large for "
        f"Oase1, which is where it changes the conclusion.\n",
        ...
        "## Reproduce\n",
        "```bash\n"
        f"python neanderthal_source.py --panel {args.panel}\n"
        f"python neanderthal_source.py --panel {args.panel} --transversions\n"
        "```\n",
...
def _transversion_note(args):
    ...
    if not (os.path.exists(p) and os.path.exists(pw)):
        return ("Not run. Execute `python neanderthal_source.py --panel 1240k "
