...
Outputs: results/simulation/*.csv, figures fig_sim_*, docs/studies/SIMULATION_VALIDATION.md,
...
def main():
    ...
    print("\nWrote docs/studies/SIMULATION_VALIDATION.md + reports/simulation_report.html")
...
def write_report(A, B, C, D, sl, ic, r, fpr, n_bonf, n_ind, zc, figs):
    ...
    md = SIM_MD.format(date=date, sl=f"{sl:.3f}", ic=f"{ic*100:+.3f}", r=f"{r:.4f}",
                       ...
                       power_table="\n".join(
                           ...
                           for _, x in D.iterrows()))
    open(os.path.join(HERE, "docs", "studies", "SIMULATION_VALIDATION.md"), "w", encoding="utf-8").write(md)
...
paper, complementing the reproduction of published values (docs/studies/VALIDATION.md).
...
<h2>Figures</h2>{figs}
<footer>Method &amp; code: <code>archaic/simulate.py</code>, <code>validate_simulation.py</code>. Companion: <code>docs/studies/SIMULATION_VALIDATION.md</code>. Refs: Kelleher 2016; Baumdicker 2022 (msprime).</footer>
</div></body></html>"""
