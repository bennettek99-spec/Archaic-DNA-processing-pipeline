...
def _common(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--source-classes", nargs="+")
...
def _select(frame: pd.DataFrame, args: argparse.Namespace) -> pd.DataFrame:
    ...
    if args.source_classes:
        if "source_class" not in selected:
            raise ValueError("--source-classes requires a source_class column")
        selected = selected.loc[selected["source_class"].isin(args.source_classes)]
...
def _input_manifest(args: argparse.Namespace) -> dict:
    ...
    if args.caller.lower() == "skov_hmm":
        manifest.update(
            {
                "dataset": "Skov et al. 2018 PLOS Genetics S5 archaic segments",
                "doi": "10.1371/journal.pgen.1007641",
                "supplement_url": (
                    "https://journals.plos.org/plosgenetics/article/file"
                    "?id=10.1371/journal.pgen.1007641.s012&type=supplementary"
                ),
                "access_authorization": "public PLOS supplementary dataset (CC BY 4.0 article)",
                "posterior_semantics": (
                    "MeanProb is retained as posterior_archaic; it is not treated as "
                    "a Denisovan-specific posterior"
                ),
                "genetic_length_conversion": (
                    "physical length multiplied by 1.2e-8 recombination events per "
                    "bp per generation and by 100 cM per Morgan"
                ),
                "source_classification": (
                    "broad Denisovan affinity requires Shared_with_Denisova > "
                    "Shared_with_Vindija; strict sensitivity additionally requires "
...
def _load_published_benchmark(output: Path) -> dict[str, Any] | None:
    path = output / "report" / "skov_s4_benchmark.json"
    if not path.exists():
        return None
...
def command_report(args: argparse.Namespace) -> int:
    ...
    _, html_path = write_report(
        ...
        published_benchmark=_load_published_benchmark(output),
    )
    print(html_path)
    return 0
...
def command_run_all(args: argparse.Namespace) -> int:
    ...
    if not (args.resume and checkpoint.unit_valid(state, "bootstrap")):
        ...
        two_bootstrap = bootstrap_fit(
            tracts,
            fit_two_pulse,
            replicates=int(config["dating"]["complex_bootstrap_replicates"]),
            seed=int(config["project"]["random_seed"]) + 702,
            group_column="chromosome",
            max_tracts=config["dating"].get("complex_bootstrap_max_tracts"),
            fitter_kwargs={
                **_fit_kwargs(config),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    import_parser.add_argument(
        "--caller",
        default="generic",
        choices=["generic", "ibdmix", "admixfrog", "hmmix", "archaicseeker2", "skov_hmm"],
    )
